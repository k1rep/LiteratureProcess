...
def load_docs_file():
    try:
        ...
        print(f"Error loading docs file: {str(e)}")
        return {}
...
def download_doc(doi):
    try:
        r = requests.get('https://dl.acm.org/doi/' + doi, timeout=30)
        r.raise_for_status()
...
def download(orig, num_docs):
    ...
    while len(docs) < num_docs:
        ...
        if uid in all_docs:
            ...
            doc = all_docs[uid]
            if doc is None:
                print('Failed to load %s, skipping.' % uid)
                break
        else:
            print('%s: Downloading %s...' % (len(docs) + 1, uid))
            doc = download_doc(uid)
            if doc is None:
                print('Failed to download %s, skipping.' % uid)
                break
            all_docs[uid] = doc
            save_doc(uid, doc)
            # save_docs(all_docs)
...
def most_referenced(docs, orig):
    ...
    for i in range(min(100, len(docs))):
        uid, cnt = rank[i]
        doc = docs[uid]
        print('%3d, %8s, %3d (%3d), %s. %s: %s' % (
            i + 1, uid, cnt, len(doc['citedby']), doc['date'] if 'date' in doc else '??/??/????',
            doc['authors'] if 'authors' in doc else '???', doc['title'] if 'title' in doc else '???'))
...
if __name__ == '__main__':
    input_file_path = 'doi_ai.xlsx'
    documents_to_download = 85
    data = pd.read_excel(input_file_path)
    for doi in data['DOI']:
        docs = download(doi, documents_to_download)
        # docs是空的
        if len(docs) == 0:
            print('No documents downloaded, skipping.')
            continue
        info(docs[doi])
        most_referenced(docs, doi)
